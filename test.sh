...
USAGE="usage: ./test.sh desired-miner-hostname domain"
MINER_HOSTNAME=${1:?"$USAGE"}
MINER_DOMAIN=${2:?"$USAGE"}
MINER_URL=https://$MINER_HOSTNAME.$MINER_DOMAIN

cf map-route miners $MINER_DOMAIN -n $MINER_HOSTNAME
trap "cf unmap-route miners $MINER_DOMAIN -n $MINER_HOSTNAME" EXIT
...
curl --data '{"method":"txpool_status","params":[],"id":0,"jsonrpc":"2.0"}' -X POST -s $MINER_URL | jq -r '.result'
echo
...
ACCOUNTS=( $(curl --data '{"method":"eth_accounts","params":[],"id":0,"jsonrpc":"2.0"}' -X POST -s $MINER_URL | jq -r '.result | to_entries[] | "\(.value)"') )
echo "Accounts: ${ACCOUNTS[@]}"

echo "Setup balances (will mine until sender has > 1,000,000):"
while true; do
  SENDER_WEI=$(curl --data '{"method":"eth_getBalance","params":["'${ACCOUNTS[0]}'","latest"],"id":0,"jsonrpc":"2.0"}' -X POST -s $MINER_URL | jq -r '.result' | hexToDec)
  RECEVR_WEI=$(curl --data '{"method":"eth_getBalance","params":["'${ACCOUNTS[1]}'","latest"],"id":0,"jsonrpc":"2.0"}' -X POST -s $MINER_URL | jq -r '.result' | hexToDec)
...
curl --data '{"method":"personal_unlockAccount","params":["'${ACCOUNTS[0]}'","password"],"id":0,"jsonrpc":"2.0"}' -X POST -s $MINER_URL | jq -r '.result'
curl --data '{"method":"eth_sendTransaction","params":[{"from":"'${ACCOUNTS[0]}'","to":"'${ACCOUNTS[1]}'","value":"'$VALUE'"}],"id":0,"jsonrpc":"2.0"}' -X POST -s $MINER_URL | jq -r '.result'
echo
...
while true; do
  PENDING_STATUS=$(curl --data '{"method":"txpool_status","params":[],"id":0,"jsonrpc":"2.0"}' -X POST -s $MINER_URL | jq -r '.result.pending')
  SENDER_WEI=$(curl --data '{"method":"eth_getBalance","params":["'${ACCOUNTS[0]}'","latest"],"id":0,"jsonrpc":"2.0"}' -X POST -s $MINER_URL | jq -r '.result' | hexToDec)
  RECEVR_WEI=$(curl --data '{"method":"eth_getBalance","params":["'${ACCOUNTS[1]}'","latest"],"id":0,"jsonrpc":"2.0"}' -X POST -s $MINER_URL | jq -r '.result' | hexToDec)
